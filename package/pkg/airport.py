            ...
            for option in self.audio_controls:
                if str(option['human_device_name']) == str(selection):
                    ...
                    try:
                        ...
                        self.change_shairport_config('//	output_device = "default";','output_device = "plughw:CARD=' + option["simple_card_name"] + ',DEV=' + option["device_id"] + '";')
                        self.change_shairport_config('//	mixer_control_name = "PCM";','//	mixer_control_name = "' + option["control_name"] + '";')
                    except Exception as ex:
...
def get_audio_controls():
    ...
    for line in lines:
        if line.startswith( 'card ' ):
            ...
            print("simple card name = " + simple_card_name.strip() )
            
            full_card_name   = re.findall(r"\[([^']+)\]", line_a)[0]
            print("full card name = " + full_card_name)
            
            full_device_name = re.findall(r"\[([^']+)\]", line_b)[0]
            print("full device name = " + full_device_name)
            
            human_device_name = full_device_name
            ...
            audio_controls.append({'card_id':card_id, 'device_id':device_id, 'simple_card_name':simple_card_name, 'full_card_name':full_card_name, 'full_device_name':full_device_name, 'human_device_name':human_device_name, 'control_name':control_name}) # ,'controls':lines
